/*
	This package implements the functionality needed to reliably get a client
	to the Mesos log interface.
*/
package mesoslogs
...
const (
	...
	PageLength int    = 5000
	TailURIFmt string = "http://%s/files/read.json?path=%s&offset=%v&length=%v"
)
...
// {host} - the host/ip:port of the mesos master node
func NewMesosClient(host string) (*MesosClient, error) {
	masterURL, err := getMasterRedirect(host)
	if err != nil {
		return nil, err
	}
	...
	return &MesosClient{Host: host, MasterURL: masterURL, State: state}, nil
}
...
func getMasterRedirect(host string) (string, error) {
	url := fmt.Sprintf("http://%s/master/redirect", host)
